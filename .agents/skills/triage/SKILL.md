...
This minimal factory does not automate specification work. A `Ready to spec` result parks the issue for a human to handle or route outside this workflow.
...
This repository provisions four exact, title-case labels. Set `label` to the same exact string as `state`: `Ready to implement`, `Ready to spec`, `Needs info`, or `Wait to implement`. List only existing labels from that same four-label set in `remove_labels`.
